...
import { Activity, BarChart3, HelpCircle, Users } from "lucide-react"
...
const sidebarItems = [
  ...
  { href: "/admin/questions", label: "Quản lý câu hỏi", icon: HelpCircle },
]
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Button
                ...
                className="flex-1 sm:flex-none min-w-[80px] justify-center mx-1 rounded-b-none border-b-2 border-transparent data-[state=active]:border-primary hover:border-primary/50 data-[state=active]:bg-secondary/50"
              >
                <Link href={item.href} className="flex flex-col items-center gap-1 py-1">
                  <Icon className="h-4 w-4" />
                  <span className="text-[10px] md:text-sm text-center">{item.label}</span>
                </Link>
              </Button>
            )
          })}
